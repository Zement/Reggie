...
class PatchManagerDialog(QtWidgets.QDialog):
    ...
    def _load_plugin_editor(self, patch):
        ...
        # Available plugins come from the single registry so this UI and the
        # default plugins.xml can never drift. See reggie/plugins/patch_plugins.py.
        from reggie.plugins.patch_plugins import REGISTRY
        available_plugins = [
            {
                'id': p.id,
                'name': p.display_name,
                ...
                'params': [
                    {'name': pr.name, 'label': pr.label, 'default': pr.default}
                    for pr in p.params
                ],
            }
            for p in REGISTRY
            ...
            if riiv_xml_url:
                ...
                if riiv_xml_subfolder:
                    ...
                    try:
                        ...
                        from reggie.patches.download_manager import normalize_catalog_url
