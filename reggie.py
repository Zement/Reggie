...
# Quick Paint Tool - Will be imported in main() after QApplication is created
# Don't import here - it causes issues with the import chain
QPT_AVAILABLE = True
QPT_INITIALIZED = False
_qpt_functions = None  # Will be populated in main()
...
class ReggieWindow(QtWidgets.QMainWindow):
    ...
    def __init2__(self):
        ...
        if QPT_AVAILABLE and not QPT_INITIALIZED and _qpt_functions:
            ...
            print(f"[INIT2] QPT not available (QPT_AVAILABLE={QPT_AVAILABLE}, QPT_INITIALIZED={QPT_INITIALIZED}, _qpt_functions={_qpt_functions is not None})")
...
def main():
    ...
    # Import all deferred modules now that QApplication exists
    global GetIcon, SetAppStyle, ListWidgetWithToolTipSignal, LoadNumberFont, LoadTheme, IconsOnlyTabBar
    global GameDefMenu, LoadGameDef
    global PatchManagerDialog, BGDialog, ZonesDialog, AreaOptionsDialog
    global Stamp, StampChooserWidget, SpriteList, SpritePickerWidget, ObjectPickerWidget, LevelOverviewWidget
    global SpriteEditorWidget
    
    print("[BOOT] Importing deferred modules...")
    try:
        print("[BOOT] Importing ui...")
        from ui import GetIcon, SetAppStyle, ListWidgetWithToolTipSignal, LoadNumberFont, LoadTheme, IconsOnlyTabBar
        print("[BOOT] ✓ ui imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing ui: {e}")
        raise
    
    try:
        print("[BOOT] Importing gamedef...")
        from gamedef import GameDefMenu, LoadGameDef
        print("[BOOT] ✓ gamedef imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing gamedef: {e}")
        raise
    
    try:
        print("[BOOT] Importing patch_manager_dialog...")
        from patch_manager_dialog import PatchManagerDialog
        print("[BOOT] ✓ patch_manager_dialog imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing patch_manager_dialog: {e}")
        raise
    
    try:
        print("[BOOT] Importing background...")
        from background import BGDialog
        print("[BOOT] ✓ background imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing background: {e}")
        raise
    
    try:
        print("[BOOT] Importing zones...")
        from zones import ZonesDialog
        print("[BOOT] ✓ zones imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing zones: {e}")
        raise
    
    try:
        print("[BOOT] Importing area...")
        from area import AreaOptionsDialog
        print("[BOOT] ✓ area imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing area: {e}")
        raise
    
    try:
        print("[BOOT] Importing sidelists...")
        from sidelists import Stamp, StampChooserWidget, SpriteList, SpritePickerWidget, ObjectPickerWidget, LevelOverviewWidget
        print("[BOOT] ✓ sidelists imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing sidelists: {e}")
        raise
    
    try:
        print("[BOOT] Importing spriteeditor...")
        from spriteeditor import SpriteEditorWidget
        print("[BOOT] ✓ spriteeditor imported")
    except Exception as e:
        print(f"[BOOT] ✗ Error importing spriteeditor: {e}")
        raise
